...
function RegistrationCountdown() {
  ...
  return (
    ...
    >
      <div className="flex items-baseline gap-0.5 sm:gap-1.5 font-pixel font-bold text-amber-300 tabular-nums"
...
export default function Navbar({ soundEnabled, toggleChiptune }) {
  ...
  return (
    <header className="relative z-40 w-full px-4 sm:px-6 py-3 sm:py-5 lg:px-12 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2">
      {/* Row 1 — brand left; on mobile the sound button sits top-right */}
      <div className="flex items-center justify-between w-full sm:w-auto sm:shrink-0">
        ...
        {/* Sound toggle — mobile only (top-right of row 1) */}
        <button
          className="sm:hidden w-9 h-9 flex items-center justify-center rounded-lg bg-slate-900/60 hover:bg-slate-800/80 border border-teal-500/40 text-teal-300 transition backdrop-blur-md"
      ...
      <nav className="hidden xl:flex items-center gap-2.5 bg-[#0c2626]/80 backdrop-blur-md p-2 rounded-xl border border-teal-500/30 shadow-2xl">
        {navItems.map((item) => (
          <button
            key={item.target}
            className="retro-nav-pill px-5 py-2.5 text-sm font-pixel font-semibold uppercase bg-[#184844] hover:bg-[#205e58] text-white rounded-lg border border-[#0e2c29]"
      ...
      {/* Row 2 (mobile) / right group (desktop): countdown + desktop sound */}
      <div className="flex items-center justify-center sm:justify-end gap-1.5 sm:gap-3 w-full sm:w-auto shrink-0">
        ...
        <button
          className="hidden sm:flex w-12 h-12 items-center justify-center rounded-lg bg-slate-900/60 hover:bg-slate-800/80 border border-teal-500/40 text-teal-300 transition backdrop-blur-md"
          onClick={toggleChiptune}
          title="Toggle Retro Sound"
        >
