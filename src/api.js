  ...
  if (path === '/api/teams/create') {
    ...
    const newTeam = {
      ...
      payment: {
        status: 'submitted',
        utr: String(utr).trim(),
        amount: Number(body.payment?.amount) || 600,
  ...
  if (path === '/api/ops/verify-payment') {
    ...
    return {
      ...
      emailDispatched: {
        ...
        body: `Greetings ${targetTeam.leader?.name || 'Leader'},\n\nYour payment of ₹${targetTeam.payment?.amount || 600} (UTR: ${targetTeam.payment?.utr}) has been successfully verified against our accounts. Your squad "${targetTeam.name}" is officially confirmed for Codefiesta 5.0.\n\nAll candidate passes and entry QR codes are now active on your dashboard.\n\nCodefiesta 5.0 HQ`,
        utr: targetTeam.payment?.utr,
        amount: targetTeam.payment?.amount || 600,
